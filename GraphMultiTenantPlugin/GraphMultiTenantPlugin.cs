...
namespace GraphMultiTenantPlugin;

public class GraphMultiTenantPlugin
{
    public List<string> Tenants { get; set; } = new();
}

public class GraphMultiTenantPlugin(IPluginEvents pluginEvents, IProxyContext context, ILogger logger, ISet<UrlToWatch> urlsToWatch, IConfigurationSection? configSection = null) : BaseProxyPlugin(pluginEvents, context, logger, urlsToWatch, configSection)
{
    public override string Name => nameof(GraphMultiTenantPlugin);
